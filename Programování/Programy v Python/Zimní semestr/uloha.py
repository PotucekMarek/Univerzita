...
def get_table_cell(table, row_number, column):
    ...
    result = get_row_cell(row, column)          # funce get_row_cell vrací buňku na základě zadaného sloupce a řádku
    return result
...
def print_table(table):
    row_number = get_table_row_count(table)
    for i in range(row_number):
        row = get_table_row(table, i)
        print_row(row)
        print()
...
def join_rows(row1, row2):
    lst = [row1 + row2]                         # do prázdného seznamu vloží dva řádky
    return lst
...
def add_row_to_table(table, row):
    lst = table + [row]                         # do proměnné se vloží tabulka, přičte řádek
    return lst

#add_row_to_table([[1, 2], [3, 4]], [5, 6])
...
def product(table1, table2):
    row1 = make_empty_table()
    row2 = make_empty_table()
    for i in range(get_table_row_count(table1)):
        for j in range(get_table_row_count(table2)):
            row1 = [get_table_row(table1, i)]
            row2 = [get_table_row(table2, j)]
            table = join_rows(row1, row2)
            print_table(table)
product([[1, 20], [2, 3], [3, 1]], [['a', True], ['b', False]])