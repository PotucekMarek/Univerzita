  ...
  <div class="navbar-nav">
    <div class="nav-item text-nowrap d-flex"> 
      ...
      if (isset($_SESSION['username'])) {
        echo '<span class="nav-link px-3">Hello, ' . htmlspecialchars($_SESSION['username']) . '</span>'; //if user is logged in, print greeting
        echo '<a class="nav-link px-3" href="logout.php">logout</a>'; // prints logout, if user is logged in
      } 

      else {
        echo '<a class="nav-link px-3" href="login.php">login</a>';
      }
      ?>
    </div>
  </div>
